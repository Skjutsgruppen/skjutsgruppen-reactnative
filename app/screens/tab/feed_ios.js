...
const circleSize = windowWidth * 0.78;
  ...
  renderFeed() {
    ...
    return (
      <FlatList
        ...
        ListFooterComponent={
          feeds.loading || (feeds.networkStatus === 1 && feeds.rows.length < 1) ?
            <Loading style={{ paddingVertical: 24 }} />
            : null
        }
        renderItem={this.renderItem}
        onEndReached={() => {
          ...
          if (feeds.loading || feeds.rows.length >= feeds.count) {
            return;
          }

          feeds.fetchMore(fetchMoreOptions);
        }}
        onScroll={(event) => {
          yPos = event.nativeEvent.contentOffset.y;
          this.circle.setNativeProps({ top: -(circleSize * 0.17) - yPos });
        }}
